...
func (c *ContentProblemService) Create(cup *goexam.ContentProblem) (err error) {
...
// Delete is
func (c *ContentProblemService) Delete(id uint) (err error) {
...
// Update is
func (c *ContentProblemService) Update(cup *goexam.ContentProblem) (err error) {
	err = c.db.Model(cup).Updates(cup).Error
	return err
}

// GetContentProblemIds is 获取所有考试的所有题目
func (c *ContentProblemService) GetContentProblemIds(contentID uint) (problemIds []uint, err error) {
	cups := make([]*goexam.ContentProblem, 0)
	err = c.db.Where("content_id = ?", contentID).Find(&cups).Pluck("problem_id", problemIds).Error
	return problemIds, err
}

// AddContentProblems is 添加考试题目
func (c *ContentProblemService) AddContentProblems(contentID uint, problemIds []uint) (err error) {
...
func (c *ContentProblemService) UpdateContentProblems(contentID uint, newProblemIds []uint) (err error) {
	contentProblem := new(goexam.ContentProblem)
...
func (c *ContentProblemService) DeleteByContentID(contentID uint) (err error) {
