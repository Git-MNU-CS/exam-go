...
func (r *RecordController) GetList(ctx echo.Context) error {
	recordFilter := new(goexam.RecordFilter)
	err := ctx.Bind(recordFilter)

	if err != nil {
		return ctx.String(http.StatusBadRequest, err.Error())
	}

	records, err := r.RecordService.GetList(ctx, recordFilter)
